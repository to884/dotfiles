# Azure Key Vault Integration (オプション機能)

> **⚠️ 注意：この機能はオプションです**
> 
> このdotfilesリポジトリの**デフォルト設定では、シークレットは手動で配置**します。  
> Azure Key Vault連携は、自動的なシークレット管理が必要な場合のみ使用してください。
> 
> 通常のセットアップ方法については、[README.md](README.md) の「🔒 セキュリティとシークレット管理」を参照してください。

---

## 概要

このドキュメントでは、Azure Key Vaultを使用して機密情報を自動的に管理する方法を説明します。  
企業環境や複数マシンでシークレットを自動同期したい場合に有用です。
